...
- [ ] Task: Expand `Workspace` to support `NSCoding` (or `Codable` equivalent for all discovered keys).
- [ ] Task: Implement `WorkspaceLayout` to manage window/palette coordination.
- [ ] Task: Add `allWorkspaces` and `systemWorkspacesAsMenu` logic to `WorkspaceManager`.
- [ ] Task: Commit Phase 2.
